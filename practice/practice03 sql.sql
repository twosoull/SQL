...
--문제1.직원들의 사번(employee_id), 이름(firt_name), 성(last_name)과 부서명(department_name)을 조회하여 
...
select em.employee_id,
       em.first_name,
       em.hire_date,
       pl.first_name as manager_name,
       pl.hire_date as manager_hiredate
from employees em, employees pl
where em.manager_id = pl.employee_id
and em.hire_date < pl.hire_date;
...
select de.department_id as 부서번호,
...
--문제9.각 사원(employee)에 대해서 사번(employee_id), 이름(first_name), 부서명(department_name), 매니저(manager)의 이름(first_name)을 조회하세요.
...
select em.employee_id as 사번,
       em.first_name as 이름,
       de.department_name as 부서명,
       pl.first_name as 매니저의이름
from employees em left outer join employees pl
